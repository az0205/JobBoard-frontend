...
const RegisterR = () => {
    ...
    const handleRegister = async(e) => {
        ...
        try {
            await API.post("/recruiter/register", { name, email, password, company });
    ...
    <nav className='space-x-6  mx-auto flex justify-between items-center p-4'>
      ...
      <Link to="/candidate/login" class='nav' className= 'mx-10 text-gray-700 hover:text-blue-600 text-2xl'>Login</Link>
        ...
        <div class = 'formPage' className='flex flex-col items-center justify-center h-[91vh] bg-gray-100'>
            ...
            <h1 className='text-3xl font-extrabold text-gray-800 mb-6 text-center'> Recruiter <span className="text-blue-600">Register</span></h1>
            <form onSubmit= {handleRegister} className='space-y-4'>
                ...
                <input type="text" placeholder="Company" ref={companyRef} required className='w-full px-4 py-2 border-2 rounded-lg border-gray-300' /><br />
            ...
            <button className='w-full mt-2 text-blue-600 hover:text-blue-800 transition cursor-pointer' onClick={() => navigate('/recruiter/login')}>Login</button>
            </div>
        </div>
        </>
    )
